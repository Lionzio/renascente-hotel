...
interface RoomCardProps {
  room: Room;
  onClick: (room: Room) => void; // <-- ADICIONAMOS ISSO AQUI
}

export const RoomCard: React.FC<RoomCardProps> = ({ room, onClick }) => {
  const getStatusDisplay = (status: string) => {
    // ... (mantenha o mesmo código do getStatusDisplay que já existe)
  ...
  return (
    <div 
      onClick={() => onClick(room)} // <-- ADICIONAMOS O EVENTO AQUI
      style={{
        ...
        cursor: "pointer", // <-- ADICIONAMOS O CURSOR DE MÃOZINHA AQUI
        transition: "transform 0.2s" // Efeitinho bacana ao passar o mouse
      }}
      onMouseOver={(e) => e.currentTarget.style.transform = "translateY(-5px)"}
      onMouseOut={(e) => e.currentTarget.style.transform = "translateY(0)"}
    >
      {/* ... Mantenha o restante do código igual (h3, spans, capacidade, etc) */}
