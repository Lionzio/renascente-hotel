...
import { EditRoomPanel, FreeRoomPanel, OccupiedRoomPanel, CheckoutPanel } from "./RoomPanels";
    ...
    <div style={overlayStyle}>
      <div style={modalStyle}>
        ...
        {room.status === "OCCUPIED" && activeStay && !isCheckoutMode && (
          <OccupiedRoomPanel 
            stay={activeStay} dailyRate={dailyRate} 
            stayDays={stayDays} stayCost={stayCost} totalAmountLive={totalAmountLive}
            onAiSubmit={(text: string) => addConsumptionAI(text)}
            onManualSubmit={handleManualConsumption}
            onCheckoutRequest={() => setIsCheckoutMode(true)}
        ...
        {room.status === "OCCUPIED" && activeStay && isCheckoutMode && (
          <CheckoutPanel 
            stay={activeStay} stayDays={stayDays} dailyRate={dailyRate} setDailyRate={setDailyRate}
            stayCost={stayCost} totalAmountLive={totalAmountLive}
            onCancel={() => setIsCheckoutMode(false)} onConfirm={handleCheckoutFinal}
            isLoading={stayLoading}
          />
        )}
      </div>
